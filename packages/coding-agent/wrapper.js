...
exports.runWrapperWithVerification = runWrapperWithVerification;
exports.main = main;
...
`;
    try {
        ...
        const { session: verificationSession } = await (0, index_js_1.createAgentSession)({
            sessionManager: index_js_1.SessionManager.inMemory(),
            authStorage,
            modelRegistry,
            model,
            thinkingLevel,
 ...
 */
async function runWrapperWithVerification(args) {
    const { prompt: originalPrompt, apiKey, model: preferredModel, maxRetries = 3, verbose = false, } = args;
    // Set up auth and model registry
    const authStorage = new index_js_1.AuthStorage();
    if (apiKey) {
        authStorage.setRuntimeApiKey("anthropic", apiKey);
    }
    const modelRegistry = new index_js_1.ModelRegistry(authStorage);
    // Create session with in-memory storage (no persistence)
    const { session } = await (0, index_js_1.createAgentSession)({
        sessionManager: index_js_1.SessionManager.inMemory(),
        authStorage,
        modelRegistry,
    });
