 ...
 *
 * This is a copy of the parser from @mariozechner/pi-ai to allow
 * @jbutlerdev/pi-coding-agent to be published independently with XML
 * tool call support.
 *
 ...
 * 4. Inline name with arg_key/arg_value: <tool_call><name>tool_name</name><arg_key>key</arg_key><arg_value>value</arg_value></tool_call>
 *
...
export function createXmlToolCallParser(): XmlToolCallParser {
	...
	function parseToolCallsFromText(text: string): ParsedToolCall[] {
		...
		// <tool_call><name>bash</name><arg_key>command</arg_key><arg_value>ls</arg_value></tool_call>
		const inlineNameRegex =
			/<(tool_call|function_call|invoke|call|execute)>(\w+)((?:<arg_key>[\s\S]*?<\/arg_key>[\s\S]*?<\/arg_value>)+)<\/\1>/gi;
