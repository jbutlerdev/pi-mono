 ...
 */
class AssistantMessageComponent extends pi_tui_1.Container {
    contentContainer;
    hideThinkingBlock;
    markdownTheme;
    lastMessage;
    constructor(message, hideThinkingBlock = false, markdownTheme = (0, theme_js_1.getMarkdownTheme)()) {
        super();
    ...
    updateContent(message) {
            ...
            if (content.type === "text" && content.text.trim()) {
                ...
                if (this.hideThinkingBlock) {
                    ...
                    this.contentContainer.addChild(new pi_tui_1.Markdown(content.thinking.trim(), 1, 0, this.markdownTheme, {
                        color: (text) => theme_js_1.theme.fg("thinkingText", text),
                        italic: true,
                    }));
                    if (hasVisibleContentAfter) {
                        this.contentContainer.addChild(new pi_tui_1.Spacer(1));
        ...
        if (!hasToolCalls) {
            if (message.stopReason === "aborted") {
                ...
                this.contentContainer.addChild(new pi_tui_1.Spacer(1));
                this.contentContainer.addChild(new pi_tui_1.Text(theme_js_1.theme.fg("error", `Error: ${errorMsg}`), 1, 0));
            }
        }
    }
}
