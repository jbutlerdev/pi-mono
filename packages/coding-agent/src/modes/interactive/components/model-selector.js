 ...
 */
class ModelSelectorComponent extends pi_tui_1.Container {
    searchInput;
    ...
    constructor(tui, currentModel, settingsManager, modelRegistry, scopedModels, onSelect, onCancel, initialSearchInput) {
        ...
        if (scopedModels.length > 0) {
            this.scopeText = new pi_tui_1.Text(this.getScopeText(), 0, 0);
            this.addChild(this.scopeText);
            this.scopeHintText = new pi_tui_1.Text(this.getScopeHintText(), 0, 0);
            this.addChild(this.scopeHintText);
        }
        else {
            const hintText = "Only showing models with configured API keys (see README for details)";
            this.addChild(new pi_tui_1.Text(theme_js_1.theme.fg("warning", hintText), 0, 0));
        }
        this.addChild(new pi_tui_1.Spacer(1));
        // Create search input
    ...
    sortModels(models) {
        ...
        sorted.sort((a, b) => {
            const aIsCurrent = (0, pi_ai_1.modelsAreEqual)(this.currentModel, a.model);
            const bIsCurrent = (0, pi_ai_1.modelsAreEqual)(this.currentModel, b.model);
    ...
    getScopeText() {
        const allText = this.scope === "all" ? theme_js_1.theme.fg("accent", "all") : theme_js_1.theme.fg("muted", "all");
        const scopedText = this.scope === "scoped" ? theme_js_1.theme.fg("accent", "scoped") : theme_js_1.theme.fg("muted", "scoped");
        return `${theme_js_1.theme.fg("muted", "Scope: ")}${allText}${theme_js_1.theme.fg("muted", " | ")}${scopedText}`;
    }
    getScopeHintText() {
    ...
    filterModels(query) {
        this.filteredModels = query
            ? (0, pi_tui_1.fuzzyFilter)(this.activeModels, query, ({ id, provider }) => `${id} ${provider}`)
            : this.activeModels;
        this.selectedIndex = Math.min(this.selectedIndex, Math.max(0, this.filteredModels.length - 1));
        this.updateList();
    }
    updateList() {
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            const isCurrent = (0, pi_ai_1.modelsAreEqual)(this.currentModel, item.model);
            let line = "";
            if (isSelected) {
                const prefix = theme_js_1.theme.fg("accent", "→ ");
                const modelText = `${item.id}`;
                const providerBadge = theme_js_1.theme.fg("muted", `[${item.provider}]`);
                const checkmark = isCurrent ? theme_js_1.theme.fg("success", " ✓") : "";
                line = `${prefix + theme_js_1.theme.fg("accent", modelText)} ${providerBadge}${checkmark}`;
            }
            else {
                const modelText = `  ${item.id}`;
                const providerBadge = theme_js_1.theme.fg("muted", `[${item.provider}]`);
        ...
        if (startIndex > 0 || endIndex < this.filteredModels.length) {
            const scrollInfo = theme_js_1.theme.fg("muted", `  (${this.selectedIndex + 1}/${this.filteredModels.length})`);
