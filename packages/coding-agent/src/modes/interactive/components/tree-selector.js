...
class TreeList {
    ...
    constructor(tree, currentLeafId, maxVisibleLines, initialSelectedId) {
        ...
        const targetId = initialSelectedId !== null && initialSelectedId !== void 0 ? initialSelectedId : currentLeafId;
    ...
    applyFilter() {
        ...
        if (this.filteredNodes.length > 0) {
            this.lastSelectedId = (_c = (_a = this.filteredNodes[this.selectedIndex]) === null || _a === void 0 ? void 0 : _a.node.entry.id) !== null && _c !== void 0 ? _c : this.lastSelectedId;
        }
        ...
        if (this.filteredNodes.length > 0) {
            this.lastSelectedId = (_d = (_b = this.filteredNodes[this.selectedIndex]) === null || _b === void 0 ? void 0 : _b.node.entry.id) !== null && _d !== void 0 ? _d : this.lastSelectedId;
        }
    }
    /**
    ...
    recalculateVisualStructure() {
        ...
        const findVisibleAncestor = (nodeId) => {
            var _a, _b;
            var _c, _d;
            let currentId = (_c = (_a = entryMap.get(nodeId)) === null || _a === void 0 ? void 0 : _a.node.entry.parentId) !== null && _c !== void 0 ? _c : null;
            while (currentId !== null) {
                if (visibleIds.has(currentId)) {
                    return currentId;
                }
                currentId = (_d = (_b = entryMap.get(currentId)) === null || _b === void 0 ? void 0 : _b.node.entry.parentId) !== null && _d !== void 0 ? _d : null;
            }
            return null;
        };
    ...
    render(width) {
        ...
        if (this.filteredNodes.length === 0) {
            ...
            lines.push((0, pi_tui_1.truncateToWidth)(theme_js_1.theme.fg("muted", `  (0/0)${this.getFilterLabel()}`), width));
            return lines;
        }
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            const pathMarker = isOnActivePath ? theme_js_1.theme.fg("accent", "• ") : "";
            const label = flatNode.node.label ? theme_js_1.theme.fg("warning", `[${flatNode.node.label}] `) : "";
            const content = this.getEntryDisplayText(flatNode.node, isSelected);
            let line = cursor + theme_js_1.theme.fg("dim", prefix) + pathMarker + label + content;
            if (isSelected) {
                line = theme_js_1.theme.bg("selectedBg", line);
            }
            lines.push((0, pi_tui_1.truncateToWidth)(line, width));
        }
        lines.push((0, pi_tui_1.truncateToWidth)(theme_js_1.theme.fg("muted", `  (${this.selectedIndex + 1}/${this.filteredNodes.length})${this.getFilterLabel()}`), width));
        return lines;
    }
            ...
            case "message": {
                ...
                if (role === "user") {
                    ...
                    if (toolCall) {
                        ...
                        result = theme_js_1.theme.fg("muted", `[${(_a = toolMsg.toolName) !== null && _a !== void 0 ? _a : "tool"}]`);
                    }
                }
                else if (role === "bashExecution") {
                    const bashMsg = msg;
                    result = theme_js_1.theme.fg("dim", `[bash]: ${normalize((_b = bashMsg.command) !== null && _b !== void 0 ? _b : "")}`);
                }
                else {
                    result = theme_js_1.theme.fg("dim", `[${role}]`);
                }
                break;
            }
            case "custom_message": {
                const content = typeof entry.content === "string"
                    ...
                    : entry.content
                        ...
                        .map((c) => c.text)
                        .join("");
                result = theme_js_1.theme.fg("customMessageLabel", `[${entry.customType}]: `) + normalize(content);
                break;
            }
            case "compaction": {
                const tokens = Math.round(entry.tokensBefore / 1000);
                result = theme_js_1.theme.fg("borderAccent", `[compaction: ${tokens}k tokens]`);
                break;
            }
            case "branch_summary":
                ...
                break;
            case "model_change":
                result = theme_js_1.theme.fg("dim", `[model: ${entry.modelId}]`);
                break;
            case "thinking_level_change":
                result = theme_js_1.theme.fg("dim", `[thinking: ${entry.thinkingLevel}]`);
                break;
            case "custom":
                result = theme_js_1.theme.fg("dim", `[custom: ${entry.customType}]`);
                break;
            case "label":
                result = theme_js_1.theme.fg("dim", `[label: ${(_c = entry.label) !== null && _c !== void 0 ? _c : "(cleared)"}]`);
                break;
            default:
                result = "";
        }
        ...
        switch (name) {
            case "read": {
                ...
                if (offset !== undefined || limit !== undefined) {
                    const start = offset !== null && offset !== void 0 ? offset : 1;
...
class SearchLine {
    ...
    render(width) {
        ...
        if (query) {
            return [(0, pi_tui_1.truncateToWidth)(`  ${theme_js_1.theme.fg("muted", "Type to search:")} ${theme_js_1.theme.fg("accent", query)}`, width)];
        }
        return [(0, pi_tui_1.truncateToWidth)(`  ${theme_js_1.theme.fg("muted", "Type to search:")}`, width)];
    }
...
class LabelInput {
    ...
    render(width) {
        ...
        lines.push((0, pi_tui_1.truncateToWidth)(`${indent}${theme_js_1.theme.fg("muted", "Label (empty to remove):")}`, width));
        lines.push(...this.input.render(availableWidth).map((line) => (0, pi_tui_1.truncateToWidth)(`${indent}${line}`, width)));
        lines.push((0, pi_tui_1.truncateToWidth)(`${indent}${(0, keybinding_hints_js_1.keyHint)("selectConfirm", "save")}  ${(0, keybinding_hints_js_1.keyHint)("selectCancel", "cancel")}`, width));
        return lines;
    }
    handleInput(keyData) {
        var _a, _b;
 ...
 */
class TreeSelectorComponent extends pi_tui_1.Container {
    treeList;
    labelInput = null;
    labelInputContainer;
    treeContainer;
    ...
    constructor(tree, currentLeafId, terminalHeight, onSelect, onCancel, onLabelChange, initialSelectedId) {
        ...
        this.treeContainer = new pi_tui_1.Container();
        this.treeContainer.addChild(this.treeList);
        this.labelInputContainer = new pi_tui_1.Container();
        this.addChild(new pi_tui_1.Spacer(1));
        this.addChild(new dynamic_border_js_1.DynamicBorder());
        this.addChild(new pi_tui_1.Text(theme_js_1.theme.bold("  Session Tree"), 1, 0));
        this.addChild(new pi_tui_1.TruncatedText(theme_js_1.theme.fg("muted", "  ↑/↓: move. ←/→: page. Shift+L: label. ") +
    ...
    showLabelInput(entryId, currentLabel) {
        ...
        this.labelInput.onSubmit = (id, label) => {
            ...
            (_a = this.onLabelChangeCallback) === null || _a === void 0 ? void 0 : _a.call(this, id, label);
            this.hideLabelInput();
        };
