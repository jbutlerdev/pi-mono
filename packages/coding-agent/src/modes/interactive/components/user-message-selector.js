...
class UserMessageList {
    ...
    render(width) {
        ...
        for (let i = startIndex; i < endIndex; i++) {
            ...
            const truncatedMsg = (0, pi_tui_1.truncateToWidth)(normalizedMessage, maxMsgWidth);
            const messageLine = cursor + (isSelected ? theme_js_1.theme.bold(truncatedMsg) : truncatedMsg);
            lines.push(messageLine);
            // Second line: metadata (position in history)
            const position = i + 1;
            const metadata = `  Message ${position} of ${this.messages.length}`;
            const metadataLine = theme_js_1.theme.fg("muted", metadata);
            lines.push(metadataLine);
            lines.push(""); // Blank line between messages
        }
        // Add scroll indicator if needed
        if (startIndex > 0 || endIndex < this.messages.length) {
            const scrollInfo = theme_js_1.theme.fg("muted", `  (${this.selectedIndex + 1}/${this.messages.length})`);
 ...
 */
class UserMessageSelectorComponent extends pi_tui_1.Container {
    messageList;
    constructor(messages, onSelect, onCancel) {
        super();
        // Add header
        this.addChild(new pi_tui_1.Spacer(1));
        this.addChild(new pi_tui_1.Text(theme_js_1.theme.bold("Branch from Message"), 1, 0));
        this.addChild(new pi_tui_1.Text(theme_js_1.theme.fg("muted", "Select a message to create a new branch from that point"), 1, 0));
        this.addChild(new pi_tui_1.Spacer(1));
