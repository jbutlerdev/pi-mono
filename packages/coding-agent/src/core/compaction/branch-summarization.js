...
function prepareBranchEntries(entries, tokenBudget = 0) {
    ...
    for (let i = entries.length - 1; i >= 0; i--) {
        ...
        const tokens = (0, compaction_js_1.estimateTokens)(message);
...
async function generateBranchSummary(entries, options) {
    ...
    const response = await (0, pi_ai_1.completeSimple)(model, { systemPrompt: utils_js_1.SUMMARIZATION_SYSTEM_PROMPT, messages: summarizationMessages }, { apiKey, signal, maxTokens: 2048 });
