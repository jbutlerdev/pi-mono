...
function migrateToCurrentVersion(entries) {
    ...
    const version = (_a = header === null || header === void 0 ? void 0 : header.version) !== null && _a !== void 0 ? _a : 1;
...
function getDefaultSessionDir(cwd) {
    ...
    const sessionDir = (0, path_1.join)((0, config_js_1.getAgentDir)(), "sessions", safePath);
...
function isValidSessionFile(filePath) {
    try {
        ...
        const bytesRead = (0, fs_1.readSync)(fd, buffer, 0, 512, 0);
        (0, fs_1.closeSync)(fd);
...
function findMostRecentSession(sessionDir) {
    var _a;
    try {
        const files = (0, fs_1.readdirSync)(sessionDir)
            .filter((f) => f.endsWith(".jsonl"))
            .map((f) => (0, path_1.join)(sessionDir, f))
            .filter(isValidSessionFile)
            .map((path) => ({ path, mtime: (0, fs_1.statSync)(path).mtime }))
...
async function listSessionsFromDir(dir, onProgress, progressOffset = 0, progressTotal) {
    ...
    try {
        const dirEntries = await (0, promises_1.readdir)(dir);
        const files = dirEntries.filter((f) => f.endsWith(".jsonl")).map((f) => (0, path_1.join)(dir, f));
        const total = progressTotal !== null && progressTotal !== void 0 ? progressTotal : files.length;
        let loaded = 0;
        const results = await Promise.all(files.map(async (file) => {
            const info = await buildSessionInfo(file);
            loaded++;
            onProgress === null || onProgress === void 0 ? void 0 : onProgress(progressOffset + loaded, total);
            return info;
        }));
...
class SessionManager {
    ...
    newSession(options) {
        ...
        if (this.persist) {
            ...
            this.sessionFile = (0, path_1.join)(this.getSessionDir(), `${fileTimestamp}_${this.sessionId}.jsonl`);
        }
    ...
    _persist(entry) {
        ...
        if (!this.flushed) {
            for (const e of this.fileEntries) {
                (0, fs_1.appendFileSync)(this.sessionFile, `${JSON.stringify(e)}\n`);
            }
            this.flushed = true;
        }
        else {
    ...
    getBranch(fromId) {
        const path = [];
        const startId = fromId !== null && fromId !== void 0 ? fromId : this.leafId;
    ...
    createBranchedSession(leafId) {
        ...
        const timestamp = new Date().toISOString();
        const fileTimestamp = timestamp.replace(/[:.]/g, "-");
        const newSessionFile = (0, path_1.join)(this.getSessionDir(), `${fileTimestamp}_${newSessionId}.jsonl`);
        const header = {
            type: "session",
            version: exports.CURRENT_SESSION_VERSION,
            id: newSessionId,
        ...
        if (this.persist) {
            (0, fs_1.appendFileSync)(newSessionFile, `${JSON.stringify(header)}\n`);
            for (const entry of pathWithoutLabels) {
                (0, fs_1.appendFileSync)(newSessionFile, `${JSON.stringify(entry)}\n`);
            }
            // Write fresh label entries at the end
            const lastEntryId = ((_a = pathWithoutLabels[pathWithoutLabels.length - 1]) === null || _a === void 0 ? void 0 : _a.id) || null;
            ...
            for (const { targetId, label } of labelsToWrite) {
                ...
                (0, fs_1.appendFileSync)(newSessionFile, `${JSON.stringify(labelEntry)}\n`);
        ...
        const labelEntries = [];
        let parentId = ((_b = pathWithoutLabels[pathWithoutLabels.length - 1]) === null || _b === void 0 ? void 0 : _b.id) || null;
        for (const { targetId, label } of labelsToWrite) {
            const labelEntry = {
                type: "label",
    ...
    static create(cwd, sessionDir) {
        const dir = sessionDir !== null && sessionDir !== void 0 ? sessionDir : getDefaultSessionDir(cwd);
    ...
    static open(path, sessionDir) {
        ...
        const header = entries.find((e) => e.type === "session");
        const cwd = (_a = header === null || header === void 0 ? void 0 : header.cwd) !== null && _a !== void 0 ? _a : process.cwd();
        // If no sessionDir provided, derive from file's parent directory
        const dir = sessionDir !== null && sessionDir !== void 0 ? sessionDir : (0, path_1.resolve)(path, "..");
    ...
    static forkFrom(sourcePath, targetCwd, sessionDir) {
        ...
        const dir = sessionDir !== null && sessionDir !== void 0 ? sessionDir : getDefaultSessionDir(targetCwd);
        if (!(0, fs_1.existsSync)(dir)) {
        ...
        const timestamp = new Date().toISOString();
        const fileTimestamp = timestamp.replace(/[:.]/g, "-");
        const newSessionFile = (0, path_1.join)(dir, `${fileTimestamp}_${newSessionId}.jsonl`);
        ...
        (0, fs_1.appendFileSync)(newSessionFile, `${JSON.stringify(newHeader)}\n`);
    ...
    static async listAll(onProgress) {
        ...
        try {
            ...
            const dirs = entries.filter((e) => e.isDirectory()).map((e) => (0, path_1.join)(sessionsDir, e.name));
            // Count total files first for accurate progress
            let totalFiles = 0;
            const dirFiles = [];
            for (const dir of dirs) {
                try {
                    const files = (await (0, promises_1.readdir)(dir)).filter((f) => f.endsWith(".jsonl"));
                    dirFiles.push(files.map((f) => (0, path_1.join)(dir, f)));
            ...
            const results = await Promise.all(allFiles.map(async (file) => {
                const info = await buildSessionInfo(file);
                loaded++;
                onProgress === null || onProgress === void 0 ? void 0 : onProgress(loaded, totalFiles);
                return info;
            }));
            for (const info of results) {
                if (info) {
