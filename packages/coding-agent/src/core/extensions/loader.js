...
const jiti_1 = require("@mariozechner/jiti");
const _bundledPiAgentCore = __importStar(require("@mariozechner/pi-agent-core"));
const _bundledPiAi = __importStar(require("@mariozechner/pi-ai"));
const _bundledPiTui = __importStar(require("@mariozechner/pi-tui"));
// Static imports of packages that extensions may use.
// These MUST be static so Bun bundles them into the compiled binary.
// The virtualModules option then makes them available to extensions.
const _bundledTypebox = __importStar(require("@sinclair/typebox"));
const config_js_1 = require("../../config.js");
// NOTE: This import works because loader.ts exports are NOT re-exported from index.ts,
// avoiding a circular dependency. Extensions can import from @mariozechner/pi-coding-agent.
const _bundledPiCodingAgent = __importStar(require("../../index.js"));
...
function createExtensionAPI(extension, runtime, cwd, eventBus) {
    const api = {
        ...
        on(event, handler) {
            var _a;
            const list = (_a = extension.handlers.get(event)) !== null && _a !== void 0 ? _a : [];
            list.push(handler);
        ...
        registerCommand(name, options) {
            extension.commands.set(name, Object.assign({ name }, options));
        },
        registerShortcut(shortcut, options) {
            extension.shortcuts.set(shortcut, Object.assign({ shortcut, extensionPath: extension.path }, options));
        },
        registerFlag(name, options) {
            extension.flags.set(name, Object.assign({ name, extensionPath: extension.path }, options));
...
async function loadExtensionModule(extensionPath) {
    const jiti = (0, jiti_1.createJiti)(import.meta.url, Object.assign({ moduleCache: false }, (config_js_1.isBunBinary ? { virtualModules: VIRTUAL_MODULES, tryNative: false } : { alias: getAliases() })));
...
async function loadExtensions(paths, cwd, eventBus) {
    const extensions = [];
    const errors = [];
    const resolvedEventBus = eventBus !== null && eventBus !== void 0 ? eventBus : (0, event_bus_js_1.createEventBus)();
 ...
 */
async function discoverAndLoadExtensions(configuredPaths, cwd, agentDir = (0, config_js_1.getAgentDir)(), eventBus) {
    const allPaths = [];
    const seen = new Set();
